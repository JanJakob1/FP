...
def get_data_from_file(filename):
    ...
    with open(filename, 'r') as f:
        for line in f:
            ...
            print("k "+k+", w "+w+", r "+r+", c "+c)
            connections.append(int(float(k) * 5))
...
def main(filename):
    ...
    plt.savefig('RecurrentNetworks/mean_activity.pdf')
    ...
    ax.set_xlabel('connection weight [HW units]')
    ax.set_ylabel('connection number [1]')
    plt.savefig('RecurrentNetworks/CVs.pdf')
