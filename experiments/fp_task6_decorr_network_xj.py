...
w_max=15
# k_max=192
k_max = 38 # sonst loop endlos, vllt später nochmal laufen lassen
data_array=np.ones(((w_max+1)*(k_max+1), 4))
...
for w in range(w_max+1):
    for k in range(k_max+1):
        ...
        #     plt.plot(spiketrain, i*np.ones_like(spiketrain), 'k.', markersize=0.1)
        ...
        np.savetxt("cv_list.txt", cv_list)
        ...
        print("k={} and w={}".format(k, w))
        
        

        #data = np.concatenate((data,np.array([meanrate])))
        #data = np.concatenate((data,np.array([meancv])))
        data_array[index:]=np.array([int(k), int(w), meanrate, meancv])
        
        #np.savetxt(f,data)
        #f.write('\n')
np.savetxt("ourdata.txt", data_array) 
